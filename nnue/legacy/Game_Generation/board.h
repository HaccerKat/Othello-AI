...
inline double rnd_double(double l = 0, double r = 1E9) {
    if(l > r) swap(l, r);
...
pair<int, int> get_best_move(Board* position, double response_time, int randomness) {
    position->find_next_boards();
    clock_t start = clock();
    int depth = 1, eval = 0;
    cnt = 0, cntcomps = 0;
    ...
    for (auto [child, pair] : position->next_boards) {
        long double z = child->get_eval() / (50.0 * randomness);
