...
class Board:
    ...
    def find_next_boards(self):
        ...
        # print("Value Head:", self.value_head)
        # print("Policy Head:", self.policy_head)
    ...
    def get_next_board(self, gameplay=False):
        ...
        alpha = min(1.0, 4.0 / len(self.mcts_policy))
        epsilon = 0.25
        mcts_policy = (1 - epsilon) * np.array(self.mcts_policy) + epsilon * np.random.dirichlet(
            np.ones(len(self.mcts_policy)) * alpha)
