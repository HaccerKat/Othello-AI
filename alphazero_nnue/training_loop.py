...
def training_loop(generation, buffer, model):
    ...
    buffer_train_dataloader, buffer_test_dataloader = None, None
    # completely discard data from gens 0-19 (not used in buffer system)
    if generation >= 40:
        buffer_training_data, buffer_test_data = torch.utils.data.random_split(buffer[generation % 20], [0.8, 0.2])
        buffer_train_dataloader = DataLoader(buffer_training_data, batch_size=BATCH_SIZE, shuffle=True)
        buffer_test_dataloader = DataLoader(buffer_test_data, batch_size=BATCH_SIZE, shuffle=True)
    ...
    for t in range(epochs):
        ...
        if buffer_train_dataloader is not None:
            train_loop(buffer_train_dataloader, model, optimizer, BATCH_SIZE)

        # buffer dataset is 0.8 times the size of the main dataset
        test_loss = test_loop(test_dataloader, model)
        if buffer_test_dataloader is not None:
            test_loss += test_loop(buffer_test_dataloader, model) * 0.8
            test_loss /= 1.8
    ...
    # assign to buffer
    buffer[generation % 20] = training_data
    torch.save(best_nn.state_dict(), 'models/model_weights_' + str(int(generation) + 1) + '.pth')
    return best_nn, best_test_loss, patience == patience_counter

def update_elo(generation):
    control_model = load_model(NeuralNetwork, 'models/model_weights_' + str(generation - 10) + '.pth')
...
patience_exceeded_limit = 5
y_elo_list = [0]
x_elo_list = [0]
y_validation_loss_list = []
x_validation_loss_list = []
...
while True:
    ...
    bestNN, test_loss, patience_exceeded = training_loop(generation, buffer, model)
    patience_exceeded_counter += 1
    if patience_exceeded >= patience_exceeded_limit:
    ...
    generation += 1
    x_validation_loss_list.append(generation)
    y_validation_loss_list.append(test_loss)
    if generation % 10 == 0 and generation > 0:
        elo += update_elo(generation)
        x_elo_list.append(generation)
        y_elo_list.append(elo)
        plot(x_elo_list, y_elo_list, generation, 'elo')
        plot(x_validation_loss_list, y_validation_loss_list, generation, 'validation_loss')